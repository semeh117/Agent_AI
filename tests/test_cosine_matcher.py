...
from next_chapter import config
from next_chapter.matching.cosine_matcher import (
...
def test_esco_preferred_and_alternative_labels_share_one_concept():
    normalizer = get_esco_normalizer(Path(__file__).parent / "fixtures" / "esco" / "esco_skills.csv")
    preferred = normalizer.normalize("manage musical staff")
    alternative = normalizer.normalize("manage music staff")

    assert normalizer.concept_count == 3
...
def test_esco_concept_match_is_explainable_and_skips_embeddings():
    ...
    try:
        normalizer = get_esco_normalizer(Path(__file__).parent / "fixtures" / "esco" / "esco_skills.csv")
        with patch("next_chapter.matching.cosine_matcher._default_esco_normalizer", return_value=normalizer):
...
def test_explicit_alternatives_are_scored_as_one_requirement_each():
    fake = FakeEmbeddings({skill: [float(i == j) for i in range(5)]
        for j, skill in enumerate(["TensorFlow", "PyTorch", "Scikit-learn", "AWS", "Azure"])})
    result = with_fake_embeddings(fake, lambda: calculate_compatibility_cosine(
