"""Agent 2: complete CV-to-LinkedIn-to-Gmail workflow.
...
AGENT2_SYSTEM_PROMPT = """You are an expert career-matching assistant. Your task is to build a
...
def build_agent2_executor(verbose: bool = True) -> AgentExecutor:
    """Create Agent 2's native tool-calling executor."""
...
def run_agent2_job_matching(
    cv_info,
    results_count: int = 3,
    location: str = "",
) -> dict:
    """Run Agent 2 with an already-parsed ``CVInfo`` object."""
...
def run_agent2_full_auto(
    ...
    agent_result = run_agent2_job_matching(
        cv_info,
        results_count=results_count,
        location=location,
    )
    match_result = get_last_match_result() or {"ranked_jobs": []}
    return complete_agent2_delivery(
        agent_result,
        cv_info,
        match_result,
...
def run_agent2_full_auto_from_pdf(
    ...
    """Run Agent 2's complete workflow starting from a CV PDF source."""
...
# The natural Agent 2 entry point now runs the complete workflow. Call
# run_agent2_job_matching explicitly when ranking-only behavior is desired.
