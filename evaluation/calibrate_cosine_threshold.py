...
import re
from evaluation.evaluation_cases import CAPTURED_LABELS, SYNTHETIC_PROFILES

ROOT = Path(__file__).resolve().parent.parent


def load_cases():
    manifest = json.loads((ROOT / "tests/fixtures/parsers/evaluation_manifest.json").read_text(encoding="utf-8"))
    for name, expected in manifest["sha256"].items():
        if hashlib.sha256((ROOT / "tests" / "fixtures" / "parsers" / name).read_bytes()).hexdigest() != expected:
    ...
    raw = json.loads((ROOT / "tests/fixtures/parsers/linkedin_job_parser_fixture.json").read_text(encoding="utf-8"))["jobs"]
    cases, found = [], set()
...
def evaluate(cases, embeddings, threshold):
    from next_chapter.matching.cosine_matcher import _skills_match_cosine
    rows = []
    for case in cases:
...
def main():
    ...
    if args.check:
        ...
        return 0
    from next_chapter.config import get_embeddings, EMBEDDING_MODEL
    from next_chapter.matching.cosine_matcher import DEFAULT_COSINE_THRESHOLD
