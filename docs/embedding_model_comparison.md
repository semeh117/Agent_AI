...
> Historical evaluation from 25 August 2026. These figures do not describe the
> current parser/rules. Run `python -m evaluation.calibrate_cosine_threshold` to
> evaluate the current pinned fixtures and provisional labels.
> The historical iMocha row reports 100% precision with zero predicted positives;
> mathematically precision is undefined in that case, so it is not evidence of accuracy.
