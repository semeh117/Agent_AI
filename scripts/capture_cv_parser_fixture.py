...
Usage:
    python -m scripts.capture_cv_parser_fixture cv/AIengineer.pdf
    python -m scripts.capture_cv_parser_fixture cv/AIengineer.pdf --use-cache
    python -m scripts.capture_cv_parser_fixture --reparse-existing
...
from next_chapter.parsing.agent2_parser import CV_CACHE_VERSION, extract_cv_info_agent2


DEFAULT_OUTPUT = Path(__file__).resolve().parent.parent / Path("tests/fixtures/parsers/cv_parser_fixture.json")
