...
Usage:
    python -m scripts.capture_linkedin_job_parser_fixture "AI Engineer" --count 3
    python -m scripts.capture_linkedin_job_parser_fixture "AI Engineer" --location Germany
    python -m scripts.capture_linkedin_job_parser_fixture --reparse-existing
...
from next_chapter.parsing.agent2_parser import JOB_CACHE_VERSION, extract_job_requirements_agent2
from next_chapter.search.linkedin import search_jobs


DEFAULT_OUTPUT = Path(__file__).resolve().parent.parent / Path("tests/fixtures/parsers/linkedin_job_parser_fixture.json")
