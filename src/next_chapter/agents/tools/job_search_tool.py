...
job_search_tool.py
--------------
Agent-facing wrapper around src/next_chapter/search/himalayas.py's _search_jobs_core().
...
from next_chapter.search.himalayas import _search_jobs_core

# Only metadata needed for the next ReAct decision is returned to the agent.
# Full descriptions remain in next_chapter.search.himalayas._last_search_results, where
